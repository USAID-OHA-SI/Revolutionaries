...
You can load the local partner data directly from GitHub using the `vroom` package. In the code below, we create an object (url) that points to the location of the data on our GitHub repo. You can use this argument in vroom's `vroom()` to read in the data.
